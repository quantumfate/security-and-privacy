...
Holds the privacy gate (`bin/,privacy-check`'s registry lives in
system-config/docs/stores.md) and security-supporting helpers. The store
registry and the declaration/observation distinction are this repo's home
ground: read system-config/docs/stores.md before touching anything here.

## Commands

```sh
../hypr/bin/,privacy-check         # the gate itself; wired into every repo's just check
```
